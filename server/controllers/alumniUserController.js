...
const getAllAlumni = async (req, res) => {
    try {
        const alumni = await Alumni.find({ userType: 'alumni' }).select('-password');
        res.json({
            success: true,
            alumni
        });
...
const searchAlumniByCity = async (req, res) => {
    try {
        ...
        // Search in the basicInfo array for a location match
        const alumni = await Alumni.find({
            userType: 'alumni',
            'basicInfo.location': { $regex: city, $options: 'i' }
        }).select('-password');
...
const searchAlumniByDomain = async (req, res) => {
    try {
        ...
        // Search in the Academics array for degree or department match
        const alumni = await Alumni.find({
            userType: 'alumni',
            $or: [
                { 'Academics.degree': { $regex: domain, $options: 'i' } },
                { 'Academics.department': { $regex: domain, $options: 'i' } }
            ]
        }).select('-password');
...
const searchAlumniBatchmates = async (req, res) => {
    try {
        const { degree, university, enrollmentYear } = req.query;
        if (!degree || !university || !enrollmentYear) {
            return res.status(400).json({ message: "Degree, University, and Enrollment Year are required" });
        }

        // Exact match for the same batch (degree, university, and enrollment year)
        // enrollmentYear could be a full date string "2022-05-05", so we match the year part
        const yearMatch = enrollmentYear.substring(0, 4);
        ...
        const alumni = await Alumni.find({
            ...
            'Academics': {
                $elemMatch: {
                    ...
                    enrollmentYear: { $regex: new RegExp(`^${yearMatch}`) }
                }
            }
        }).select('-password');
